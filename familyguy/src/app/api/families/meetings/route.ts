...
export async function POST(request: Request) {
  try {
    ...
    const payload = verifyToken(token);
    if (!payload) return errorResponse("Unauthorized", 401);
    const { familyId, title, date, startTime, endTime, type, location, agenda } = await request.json();
    if (!familyId || !title || !date || !startTime || !endTime || !type || !location) return errorResponse("Missing required fields", 400);
    const membership = await prisma.familyMember.findUnique({ where: { familyId_userId: { familyId, userId: payload.userId }, deletedAt: null } });
    if (!membership) return errorResponse("Not a member", 403);
    const meeting = await prisma.meeting.create({
      data: { familyId, createdById: payload.userId, title, date: new Date(date), startTime, endTime, type, location, agenda: agenda || [] },
    });
